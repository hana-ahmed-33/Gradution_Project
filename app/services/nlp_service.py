...
CATEGORY_KEYWORDS: dict = {
    ...
    'Bills & Utilities': {
        ...
        'مياه فاتورة', 'فاتورة مياه',   # only when paired with فاتورة, not standalone
        'غاز', 'اسطوانة', 'اسطوانه',
        'انترنت', 'واي فاي', 'راوتر',
        'موبايل', 'تليفون', 'تلفون', 'خط موبايل',
        'ايجار', 'إيجار',
        'قسط', 'اقساط',
        ...
        'subscription', 'rent', 'bill', 'invoice',
...
KNOWN_ITEMS: dict = {
    ...
    'فاتورة مياه': 'Water Bill', 'water bill': 'Water Bill',
    'غاز': 'Gas Bill', 'gas bill': 'Gas Bill',
...
class CategoryClassifier:
    ...
    def classify(self, text: str, place: Optional[str] = None) -> str:
        ...
        try:
            ...
            if loop.is_running():
                # We're inside an async context — schedule as a task but use sync fallback
                ml_cat, ml_conf = smart_predictor.ml_classifier.predict_category(
                    text, place, None, None
                )
            else:
                ml_cat, ml_conf = loop.run_until_complete(
                    smart_predictor.predict_category(text, place, None, None, cat)
                )
